...
constexpr unsigned int dwLocalPlayer = 0xDC14CC;
constexpr unsigned int m_iHealth = 0x100;
constexpr unsigned int m_fFlags = 0x104;
constexpr unsigned int dwForceJump = 0x52878FC;
...
BOOL WINAPI DllMain(HINSTANCE instance, DWORD reason, LPVOID reserved) {
	if (reason == DLL_PROCESS_ATTACH) {
		...
		const HANDLE thread = CreateThread(nullptr, NULL, Setup, instance, NULL, nullptr);
